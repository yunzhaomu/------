...
* 1. 环境准备与数据清洗
gen treat = (type == "inner")
drop if dist_ring > 0 & dist_ring <= $thr // 排除缓冲区
...
postfile `results' year mean_treat mean_ctrl att pseudoR2_pre pseudoR2_post lr_pre lr_post ///
                  ...
                  using "${park}_aqci_inner_control.dta", replace
...
* 4. 载入结果并格式化展示
use "${park}_aqci_inner_control.dta", clear

* 最终列表预览
list year att m_bias_pre m_bias_post red_mbias med_bias_pre med_bias_post red_medbias b_pre b_post, clean

export excel using "D:/毕业论文数据/结果/${park}_aqci_inner_control.xlsx", firstrow(variables) replace
