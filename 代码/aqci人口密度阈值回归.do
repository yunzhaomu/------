...
import delimited "D:\毕业论文数据\导出表格数据\RS_park_pandas.csv", case(preserve) clear
...
postfile `memhold' obs thr $vars using "pandas_pop_results.dta", replace
...
forvalues i = 1/20 {
	foreach j in `years' {
		...
		* 1. 保留2500个内部区点 (dist_ring == 0)和大概3倍的内部区点
		...
		* 运行门限模型
		quietly threshold $y $thresh_x, ///
			threshvar(pop) regionvars(lndist) nthresholds(1) vce(robust)
...
* --- 读取 1000 次模拟的结果文件 ---
use "pandas_pop_results.dta", clear
...
outreg2 using "D:\毕业论文数据\论文图表\pandas_pop_Bootstrap_Result.doc", replace ///
    word bdec(3) tdec(3) rdec(3) ///
    title("Table: Threshold Model with 1000 Bootstrap Replications") ///
    addnote("Threshold variable: pop. Controlled variables are standardized.") ///
    addtext("Threshold","`mean_thr'","Bootstrap Reps", "1000")

* 2. 打开 Excel 准备写入
putexcel set "D:\毕业论文数据\结果\pandas_pop_Bootstrap_Result.xlsx", replace
